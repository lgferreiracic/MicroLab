<template>
	<div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
		...
		<div class="mt-4 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
			<div v-for="entry in entries" :key="entry.key" class="grid grid-cols-1 gap-2 border-b border-slate-200 p-3 last:border-b-0 sm:grid-cols-[220px_1fr] dark:border-slate-700">
...
export default defineComponent({
	name: 'ProductDetailSpecTable',
	props: {
		entries: {
			type: Array as PropType<StringEntry[]>,
			required: true
		}
	}
})
